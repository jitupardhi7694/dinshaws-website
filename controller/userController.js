...
const getForgotPassword = async (req, res, next) => {
   try {
      ...
      return res.render(`userDashboard/forgotPassword`);
   } catch (error) {
      logger.error(error);
      return next(error);
   }
...
const postForgotPassword = async (req, res, next) => {
   try {
      ...
      console.log('KEYS', token, JWT_RESET_KEY);
...
const getResetLink = (req, res) => res.render('userDashboard/resetPassword'); // end of getResetLink function
...
const getActivationLink = (req, res) => res.render('userDashboard/resendActivation');
...
const postActivationLink = async (req, res, next) => {
   try {
      ...
      const result = await sendActivationLinkEmail(req, res, next, user.email);
      req.flash(
         'success_msg',
...
const getActivateLinkHandler = async (req, res, next) => {
   try {
      ...
      req.flash('success_msg', `${decoded.email} has been activated. You can now log in.`);
      return res.redirect('/user/login');
   } catch (error) {
      if (error.name === 'TokenExpiredError') {
         req.flash('error_msg', 'The activation link has expired. Please regenerate.');
      } else if (error.name === 'JsonWebTokenError') {
         req.flash('error_msg', 'The activation link is invalid. Please regenerate.');
      } else {
         logger.error(error);
         return next(error);
      }

...
async function sendResetPasswordLink(req, res, next, email) {
   try {
      ...
      const token = await jwt.sign({ email: user.email, id: user.id }, process.env.JWT_RESET_KEY, { expiresIn: '2h' });
      ...
      const emailOptions = {
         ...
         text: `<h2>Use the link below to reset your password.</h2><br><a style="background-color: #66a3ff; color: white; padding: 1em 4em;text-decoration:none; border-radius: 10px" href="${host.PROTOCOL}://${host.HOST}${port}/user/forgotPassword/${token}"> Click here to reset password.</a>.<br><br>The link is valid only for 2 hours.<br>The link will work only once.<br><br><strong>Thanks and Regards,</strong><br><strong>Dinshaws Team</strong>`,
      };
